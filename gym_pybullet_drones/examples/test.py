...
from simulation.drone import DroneSimulation
...
def run_simulation(simulation, target_pos):
    ...
    for i in range(0, int(simulation.duration_sec * simulation.env.CTRL_FREQ)):
        obs, reward, terminated, truncated, info = simulation.env.step(action)
        
        for j in range(simulation.num_drones):
            wp_index = min(simulation.wp_counters[j], len(target_pos) - 1)
            action[j, :], _, _ = simulation.ctrl[j].computeControlFromState(control_timestep=simulation.env.CTRL_TIMESTEP,
                                                                            state=obs[j],
        ...
        for j in range(simulation.num_drones):
            simulation.logger.log(drone=j,
                                    timestamp=i / simulation.env.CTRL_FREQ,
                                    state=obs[j],
                                    control=np.hstack([target_pos[wp_index, :], simulation.init_rpys[j, :], np.zeros(6)]),
                                    payload_position=payload_position)
        
        simulation.env.render()
        if simulation.gui:
            sync(i, start_time, simulation.env.CTRL_TIMESTEP)
